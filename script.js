document.addEventListener("DOMContentLoaded", () => {
  ...
  function toggleActionButtons() {
    ...
    const hasRecords = Array.from(
      document.querySelectorAll(".registros-cell"),
    ).some((cell) => cell.children.length > 0);

    // Obtener el card-header
    const cardHeader = document.querySelector(".card-header");

    if (hasRecords) {
      actionBtnsContainer.classList.remove("d-none");
      if (cardHeader) cardHeader.classList.remove("d-none");
    } else {
      actionBtnsContainer.classList.add("d-none");
      if (cardHeader) cardHeader.classList.add("d-none");
    }
  }
    ...
    if (saveButton) {
      saveButton.addEventListener("click", () => {
        ...
        if (editingColId && editingTd) {
          ...
          let addedQuantityText =
            type === "piezas"
              ? `${quantity} piezas`
              : `${quantity} pallet${quantity !== 1 ? "s" : ""}`;
          ...
          partModal.hide();
          toggleActionButtons(); // Asegurar que los botones se actualicen al editar
        ...
        recordDiv.className =
          "d-flex justify-content-between align-items-center mb-1 p-1 border rounded bg-white shadow-sm";
        ...
        editBtn.addEventListener("click", function (e) {
          ...
          const btnTd = currentRow
            .querySelector('.btn-primary[title="Agregar"]')
            .closest("td");
          const rowQtyBoxCell = btnTd.previousElementSibling;
          const rowPartNoCell = rowQtyBoxCell.previousElementSibling;

          document.getElementById("partModalLabel").textContent =
            rowPartNoCell.textContent.trim();
          currentQtyPerBox =
            parseInt(rowQtyBoxCell.textContent.trim().split("/")[0]) || 0;
        ...
        toggleActionButtons(); // Asegurar que los botones se actualicen al agregar
